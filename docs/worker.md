...
Automatic `generate` and `live` use `VIDGEN_WORKER_PRESENTER_SOURCES_FILE`, an absolute path to a regular UTF-8 text file containing one absolute presenter source-image path per nonblank line. The manifest is limited to 64 KiB and 1 through 100 sources; blank lines are harmless, while duplicate, relative, malformed, or invalid-media entries fail closed. Production sets it once to `/home/deploy/vidgen-assets/presenter-sources.txt`. The Worker re-reads the manifest when an unassigned candidate reaches a real generation opportunity, so ordinary pool additions or removals do not need a PM2 restart.
