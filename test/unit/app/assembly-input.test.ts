...
function fakes() { const videoClient: VideoGenerationClient = { provider: 'fake-video', model: 'model', promptAssetIdentity: { basename: 'veo-prompts.json', sha256: 'c'.repeat(64), byteSize: 1 }, generateVideo: async () => ({ provider: 'fake-video', model: 'model', mimeType: 'video/mp4', bytes: new Uint8Array([1, 2, 3]) }) }; const speechClient: SpeechGenerationClient = { provider: 'fake-speech', model: 'model', voice: 'voice', generateSpeech: async () => ({ provider: 'fake-speech', model: 'model', voice: 'voice', mimeType: 'audio/wav', bytes: new Uint8Array([4, 5, 6]) }) }; return { createVideoClient: () => videoClient, createSpeechClient: () => speechClient }; }
