...
export const HEADLINE_SIDECAR_SCHEMA_VERSION = '4';
...
export interface HeadlineDryRunResult { readonly dryRun: true; readonly clipId: string; readonly presenterTextPath: string; readonly metadataPath: string; readonly speechPlanningDurationSeconds: number; }
...
export async function generateHeadlineClip(dependencies: HeadlineWorkflowDependencies): Promise<HeadlineWorkflowResult | HeadlineDryRunResult> {
  ...
  try {
    ...
    const speechPlanningDurationSeconds = getSimpleClipPlannedDurationSeconds(copy.copy.text, maxSeconds); const plan = planPresenterVideoDuration(speechPlanningDurationSeconds); progress(dependencies, `Presenter speech planning duration: ${speechPlanningDurationSeconds} seconds.`);
    if (dependencies.dryRun === true) {
      const metadata = buildHeadlineDryRunMetadata(clipId, story, copy, references.map(({ identity }) => identity), font, maxSeconds, speechPlanningDurationSeconds, plan, basename(dryRunTextPath), dependencies.engineVersion ?? VIDGEN_ENGINE_VERSION);
      try {
      ...
      return { dryRun: true, clipId, presenterTextPath: dryRunTextPath, metadataPath: dryRunMetadataPath, speechPlanningDurationSeconds };
    }
    ...
    const video = await activeVideoClient.generatePresenterVideo({ spokenText: copy.copy.text, referenceImages: references.map(({ image }) => image), maxSeconds: speechPlanningDurationSeconds }); validateVideoResult(video, plan);
    ...
    progress(dependencies, 'FFmpeg finishing starting.'); const finished = await finisher.finish({ rawPresenterVideoPath: rawPath, fontPath, headline: lowerThird.headline, sourceDisplayName: lowerThird.sourceDisplayName, workDirectory, outputPath: candidatePath }); progress(dependencies, 'FFmpeg finishing completed.');
    const bytes = await readFile(candidatePath); const sidecar = buildHeadlineSidecar(clipId, story, copy, video, activeVideoClient.promptAssetIdentity, references.map(({ identity }) => identity), font, maxSeconds, speechPlanningDurationSeconds, finished.rawProbe.durationSeconds, finished.probe.durationSeconds, basename(rawVeoPath), video.bytes, basename(finalPath), bytes, finished.ffmpegVersion, dependencies.engineVersion ?? VIDGEN_ENGINE_VERSION); validateHeadlineSidecar(sidecar);
...
function buildHeadlineDryRunMetadata(clipId: string, story: StoryInput, copy: SimpleClipCopyResult, references: readonly ReferenceImageIdentity[], font: FileIdentity, maxSeconds: number, speechPlanningDurationSeconds: number, plan: ReturnType<typeof planPresenterVideoDuration>, presenterTextFilename: string, engineVersion: string) {
  return {
...
export function validateHeadlineSidecar(value: unknown): void {
  ...
  rejectSidecarExtra(sidecar, ['schemaVersion', 'clipId', 'article', 'profile', 'publication', 'story', 'presenterText', 'requestedMaxSeconds', 'speechPlanningDurationSeconds', 'finalDurationSeconds', 'rawVeo', 'final', 'textProvider', 'videoProvider', 'references', 'font', 'finishing', 'engineVersion']);
  ...
  const requested = sidecar.requestedMaxSeconds; const speechPlanning = sidecar.speechPlanningDurationSeconds; const final = sidecar.finalDurationSeconds;
  if (!safeInteger(requested) || requested < 4 || requested > 20 || !safeInteger(speechPlanning) || speechPlanning < 4 || speechPlanning > SIMPLE_CLIP_REALIZABLE_MAX_SECONDS || speechPlanning > requested || typeof final !== 'number' || !Number.isFinite(final) || final <= 0) throw invalidSidecar();
  ...
  if (Math.abs(final - (sidecar.rawVeo as Record<string, number>).durationSeconds) > SIMPLE_CLIP_DURATION_TOLERANCE_SECONDS) throw invalidSidecar();
}
...
function validateRawVeo(value: unknown, clipId: unknown): void { const rawVeo = sidecarRecord(value, 'Raw Veo'); rejectSidecarExtra(rawVeo, ['filename', 'sha256', 'byteSize', 'durationSeconds']); if (rawVeo.filename !== `${clipId}.veo.mp4` || !hash(rawVeo.sha256) || !positiveInteger(rawVeo.byteSize) || typeof rawVeo.durationSeconds !== 'number' || !Number.isFinite(rawVeo.durationSeconds) || rawVeo.durationSeconds <= 0) throw invalidSidecar(); }
