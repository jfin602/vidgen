...
export function validateHeadlineSidecar(value: unknown): void {
  ...
  rejectSidecarExtra(sidecar, ['schemaVersion', 'clipId', 'article', 'profile', 'publication', 'story', 'presenterText', 'requestedMaxSeconds', 'plannedDurationSeconds', 'finalDurationSeconds', 'final', 'textProvider', 'videoProvider', 'references', 'font', 'finishing', 'engineVersion']);
  ...
  validateArticle(sidecar.article); validateProfile(sidecar.profile); validatePublication(sidecar.publication); validateStory(sidecar.story); validateProvider(sidecar.textProvider); validateVideoProvider(sidecar.videoProvider); validateReferences(sidecar.references); validateFile(sidecar.font); validateFinishing(sidecar.finishing); validateFinal(sidecar.final, sidecar.clipId);
}
