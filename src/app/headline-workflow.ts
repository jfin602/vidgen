...
export async function generateHeadlineClip(dependencies: HeadlineWorkflowDependencies): Promise<HeadlineWorkflowResult> {
  ...
  const clipId = safeClipId((dependencies.createClipId ?? randomUUID)()); const root = resolve(dependencies.artifactsRoot ?? DEFAULT_HEADLINE_ARTIFACTS_ROOT); const finalPath = join(root, `${clipId}.mp4`); const metadataPath = join(root, `${clipId}.json`); const workDirectory = join(root, `.tmp-${clipId}`); const plan = planPresenterVideoDuration(maxSeconds); const plannedDurationSeconds = Math.min(maxSeconds, plan.rawProviderDurationSeconds);
  ...
  try {
    ...
    const finished = await finisher.finish({ rawPresenterVideoPath: rawPath, fontPath, headline: lowerThird.headline, sourceDisplayName: lowerThird.sourceDisplayName, maxSeconds, plannedDurationSeconds, workDirectory, outputPath: candidatePath }); await rename(candidatePath, finalPath); published = true;
    const bytes = await readFile(finalPath); const sidecar = buildHeadlineSidecar(clipId, story, copy, video, references.map(({ identity }) => identity), font, maxSeconds, plannedDurationSeconds, finished.probe.durationSeconds, basename(finalPath), bytes, finished.ffmpegVersion, dependencies.engineVersion ?? VIDGEN_ENGINE_VERSION);
