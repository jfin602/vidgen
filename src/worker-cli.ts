...
import { createWorkerRuntimeConfig, DEFAULT_DAILY_GENERATION_LIMIT, DEFAULT_GENERATION_ATTEMPT_LIMIT, DEFAULT_WORKER_MAX_SECONDS, DEFAULT_WORKER_STATE_ROOT } from './worker/config.ts';
...
Usage:
  vidgen-worker <observe|generate|live> [--once] [--process-existing] [--max-candidates <n>] [--state-root <directory>] [--poll-interval-ms <milliseconds>] [--anchor-reference <image-path> ...] [--font-file <font-path>] [--max-seconds <4-20>] [--daily-generation-limit <n>] [--generation-attempt-limit <n>]
...
export interface WorkerCommand { readonly mode: WorkerMode; readonly once: boolean; readonly processExisting: boolean; readonly maxCandidates: number; readonly stateRoot?: string; readonly pollIntervalMs?: number; readonly anchorReferencePaths?: readonly string[]; readonly fontPath?: string; readonly maxSeconds?: number; readonly dailyGenerationLimit?: number; readonly generationAttemptLimit?: number; }
...
export function parseWorkerCliArgs(args: readonly string[]): WorkerCommand | WorkerLabelCommand | WorkerReportCommand | { readonly kind: 'help' } {
  ...
  let once = false; let processExisting = false; let maxCandidates = 1; const anchors: string[] = []; const values: Partial<Record<'stateRoot' | 'pollIntervalMs' | 'fontPath' | 'maxSeconds' | 'dailyGenerationLimit' | 'generationAttemptLimit', string>> = {};
  for (let index = 0; index < rest.length;) {
    ...
    if (option !== '--max-candidates' && option !== '--state-root' && option !== '--poll-interval-ms' && option !== '--anchor-reference' && option !== '--font-file' && option !== '--max-seconds' && option !== '--daily-generation-limit' && option !== '--generation-attempt-limit') throw invalid(`Unknown Worker argument: ${JSON.stringify(option)}.`);
    ...
    else { const key = option === '--state-root' ? 'stateRoot' : option === '--poll-interval-ms' ? 'pollIntervalMs' : option === '--font-file' ? 'fontPath' : option === '--max-seconds' ? 'maxSeconds' : option === '--daily-generation-limit' ? 'dailyGenerationLimit' : 'generationAttemptLimit'; if (values[key] !== undefined) throw invalid(`Worker option ${option} must not be repeated.`); values[key] = value; }
    index += 2;
  }
  ...
  createWorkerRuntimeConfig({ stateRoot: values.stateRoot, pollIntervalMs, maxSeconds, dailyGenerationLimit, generationAttemptLimit });
  return { mode, once, processExisting, maxCandidates, ...(values.stateRoot === undefined ? {} : { stateRoot: values.stateRoot }), ...(pollIntervalMs === undefined ? {} : { pollIntervalMs }), ...(anchors.length === 0 ? {} : { anchorReferencePaths: anchors }), ...(values.fontPath === undefined ? {} : { fontPath: values.fontPath }), ...(maxSeconds === undefined ? {} : { maxSeconds }), ...(dailyGenerationLimit === undefined ? {} : { dailyGenerationLimit }), ...(generationAttemptLimit === undefined ? {} : { generationAttemptLimit }) };
}
...
export async function runWorkerCli(args: readonly string[], output: WorkerCliOutput): Promise<number> {
  try {
    ...
    await runNgestWorker({ store: new WorkerStateStore(config.stateRoot), mode: command.mode, once: command.once, processExisting: command.processExisting, maxCandidates: command.maxCandidates, pollIntervalMs: config.pollIntervalMs, ...(command.anchorReferencePaths === undefined ? {} : { anchorReferencePaths: command.anchorReferencePaths }), ...(command.fontPath === undefined ? {} : { fontPath: command.fontPath }), maxSeconds: config.maxSeconds, dailyGenerationLimit: config.dailyGenerationLimit, generationAttemptLimit: config.generationAttemptLimit });
