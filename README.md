...
- Google-first generation uses Gemini Enterprise Agent Platform as the only supported Google model platform, with provider-neutral text, video, and speech boundaries retained for future replacement;
...
The implemented Phase 7 VidGen Worker owns live polling, new-Article discovery, bounded Parallel Web Momentum evaluation, automatic-production admission/cost policy, durable orchestration state, VidGen CLI invocation, and VidGen Poster fan-out. Parallel evidence is not creative StoryInput; live capabilities remain separately unqualified.
